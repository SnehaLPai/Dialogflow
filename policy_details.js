...
exports.policyBot = function policyBot(req,res) {
    console.log("yeah i am inside this function which is exported from virtualassistant.js");
    if(req.body.result.metadata['intentName'] == "OTP"){ 
        getBasicUserInfo(req,res);
       }
...
function getBasicUserInfo(req, res) {
  var policyNum = req.body.result.parameters['policyNumber'];
  query = 'select * from policy_details where policy_number = '+ policyNum;
  var output = "";
  var output_displaytext = "";
  console.log(query);
  index.connection.query(query, function(error,rows,fields){
    ...
    msgBdyJson["userId"] = "Sarvesh";
    msgBdyJson["userImage"] = "https://scontent-amt2-1.cdninstagram.com/t51.2885-19/s150x150/14262769_342114716128056_857373176_a.jpg";
    msgBdyJson["text"] = `Thanks ${rows[0].policy_owner}! As you hold more than one policy with us,
                          if you wish to get help on another policy please type "Get help on another policy".
                          Policy Chosen: ${rows[0].plan_name}
    ...
    var dataJson = {};
    dataJson["name"] = "";
    dataJson["text"] = "View Premium Receipts";
    dataJson["value"] = "";
    dataJson["type"] = "regular";
    dataJson["successMessage"] = "";
    dataJson["callBackFn"] = "View Premium Receipts";
    
    buttonJson["data"].push(dataJson);

    var dataJson = {};
    dataJson["name"] = "";
    dataJson["text"] = "View Policy Details";
    dataJson["value"] = "";
    dataJson["type"] = "regular";
    dataJson["successMessage"] = "";
    dataJson["callBackFn"] = "View Policy Details";
   
    buttonJson["data"].push(dataJson);
    
    var dataJson = {};
    dataJson["name"] = "";
    dataJson["text"] = "Pay Premium";
    dataJson["value"] = "";
    dataJson["type"] = "regular";
    dataJson["successMessage"] = "";
    dataJson["callBackFn"] = "Pay Premium";   

    buttonJson["data"].push(dataJson);           
    
    msgBdyJson["attachments"].push(buttonJson);
    output = msgBdyJson["text"];

    responseJson["msgBdy"] = msgBdyJson;
    output_displaytext = responseJson;
    console.log(JSON.stringify(output));
    console.log(JSON.stringify(output_displaytext));
    res.setHeader('Content-Type', 'application/json');
    res.send(JSON.stringify({ 'speech': JSON.stringify(output), 'displayText': JSON.stringify(output_displaytext) }));
